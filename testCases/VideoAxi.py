...
def gettingFacets(filename,isDrop):
    exe = ["./getFacet-threePhase", filename, isDrop]
...
def process_timestep(ti, placeFolder, folder, nGFS, GridsPerR, rmin, rmax, zmin, zmax, lw):
    t = 0.01 * ti
    place = f"{placeFolder}/snapshot-{t:.4f}"
    ...
    segs1 = gettingFacets(place, 'true')
...
def main():
    ...
    parser.add_argument('--nGFS', type=int, default=500, help='Number of restart files to process')
    parser.add_argument('--ZMAX', type=float, default=2.05, help='Maximum Z value')
    parser.add_argument('--RMAX', type=float, default=2.5, help='Maximum R value')
    parser.add_argument('--ZMIN', type=float, default=-0.1, help='Minimum Z value')
    parser.add_argument('--folder', type=str, default='sphereFilm', help='Place folder name')
    ...
    placeFolder = f"{args.folder}/intermediate"
    if not os.path.exists(placeFolder):
        print(f"{placeFolder} Folder not found!")
        return
